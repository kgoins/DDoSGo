...
func NewDataStream(agent_ip string, aPort string, handler_ip string, handler_port string, cpu int, mem int, bytesRecvd int, bytesSent int) DataStream {
	return DataStream{
		Agent_ip:     agent_ip,
		Agent_port:   aPort,
...
func (stream DataStream) BuildCmd() cmds.Cmd {
	return cmds.NewProcDataStreamCmd(stream.Agent_ip, stream.Agent_port, stream.Cpu, stream.Mem, stream.BytesRecvd, stream.BytesSent)
}
